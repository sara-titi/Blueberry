...
const Mobile = () => {
    ...
    useEffect(()=>{
        axios.get('/data.json').then(res=>{ setProducts(res.data.mobile) })
    })
    const history=useHistory();
    const  handelClick = (id) =>{ history.push(`/MobileProducts/MobileDetailes/${id}`) }


    const mobileList = products.map(function(product){
            if(product.id <= 6){
            return(
              ...
              <div className="container">
                    <div className="row">
                        ...
                        <div className="col">
                            ...
                            <ShowMore to ='/MobileProducts'>Show More </ShowMore>
                        </div>
                    </div>{/**end row */}
